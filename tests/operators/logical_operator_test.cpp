...
TEST(LogicalOperatorTest, SourceTypeName) {
    auto op = LogicalOperator(OperatorId(u64(1)), "my_source", u64(1), SourceLogicalOperatorImpl{});
...
TEST(LogicalOperatorTest, SinkTypeName) {
    auto op = LogicalOperator(OperatorId(u64(2)), "my_sink", u64(2), SinkLogicalOperatorImpl{});
...
TEST(LogicalOperatorTest, MapTypeName) {
    ...
    auto op = LogicalOperator(OperatorId(u64(3)), "mapper", u64(3), MapLogicalOperatorImpl(lambda));
...
TEST(LogicalOperatorTest, FilterTypeName) {
    ...
    auto op =
        LogicalOperator(OperatorId(u64(4)), "filter", u64(1), FilterLogicalOperatorImpl(lambda));
...
TEST(LogicalOperatorTest, StoredInVector) {
    ...
    ops.emplace_back(OperatorId(u64(1)), "src", u64(1), SourceLogicalOperatorImpl{});
    ops.emplace_back(OperatorId(u64(2)), "map", u64(2),
                     MapLogicalOperatorImpl([](Event<Record>& e) {
                         return Event<Record>(e.payload(), e.timestamp());
                     }));
    ops.emplace_back(OperatorId(u64(3)), "sink", u64(1), SinkLogicalOperatorImpl{});
...
TEST(LogicalOperatorTest, IdRoundTrip) {
    ...
    auto op = LogicalOperator(id, "myop", u64(1), SourceLogicalOperatorImpl{});
    EXPECT_EQ(op.id(), id);
}
